    ...
    <section id="prices" className="mx-auto max-w-6xl px-4 py-20 sm:px-6">
      ...
      <div className="mt-10 overflow-x-auto rounded-2xl border border-stone-200/80 bg-white shadow-sm">
        <table className="w-full min-w-[700px] border-collapse text-left text-sm">
          <thead>
            <tr className="bg-[#10233F] text-white">
          ...
          <tbody className="divide-y divide-stone-100">
            {tours.map((tour, i) => (
              <tr
                key={tour.id}
                className={`transition hover:bg-[#EEE7FF]/30 ${
                  tour.featured ? "bg-[#FFF8F1] font-medium" : i % 2 ? "bg-stone-50/60" : ""
                }`}
              >
                <td className="px-6 py-4 font-semibold text-[#10233F]">{tour.title}</td>
                <td className="px-6 py-4 font-extrabold text-[#F04483]">
                ...
                <td className="px-6 py-4 text-right">
                  <a
                    ...
                    className="inline-flex rounded-xl bg-gradient-to-r from-[#F04483] to-[#d93d74] px-4 py-2 text-xs font-bold text-white shadow-sm shadow-[#F04483]/20 transition-all duration-200 hover:scale-[1.03] hover:shadow-md hover:shadow-[#F04483]/30 hover:brightness-105 active:scale-[0.98]"
                  >
                    {s.bookLabel}
                  </a>
                </td>
